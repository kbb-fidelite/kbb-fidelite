...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('  STRIPE_SECRET_KEY:', stripeKey ? 'OK len=' + stripeKey.length : 'MANQUANTE');
    console.log('  SUPABASE_URL:', supaUrl ? 'OK' : 'MANQUANTE');
    console.log('  SERVICE_ROLE_KEY:', supaKey ? 'OK len=' + supaKey.length : 'MANQUANTE');
    ...
    console.log('Etape 4: SELECT commande Supabase');
    ...
    if (orders && orders.length > 0) {
      ...
      console.log('Etape 5: UPDATE id=', order.id, 'pending_payment → en_attente');
      ...
      console.log('  UPDATE OK:', updated ? 'activé' : 'race-condition');
    ...
    console.warn('Etape 5: FALLBACK INSERT');
    ...
    console.error('ERREUR_DETECTEE:', (error as Error).message);
    console.error('STACK:', (error as Error).stack);
    return new Response(JSON.stringify({ error: (error as Error).message, stack: (error as Error).stack }), { status: 500, headers: corsHeaders });
  }
});
