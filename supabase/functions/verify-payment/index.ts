console.log('--- START --- verify-payment chargé');
...
console.log('--- HANDLER ENREGISTREMENT ---');

Deno.serve(async (req) => {
  console.log('Etape 1: Headers OK — méthode:', req.method);
  ...
  try {
    ...
    console.log('  session_id reçu:', session_id ?? 'ABSENT');

    if (!session_id || typeof session_id !== 'string') {
      return new Response(JSON.stringify({ error: 'session_id manquant' }), { status: 400, headers: corsHeaders });
    }

    console.log('Etape 3: Tentative Stripe session retrieve — id:', session_id);
    const stripe = new Stripe(stripeKey, { apiVersion: '2024-04-10' });
    let session: Stripe.Checkout.Session;
    try {
      session = await stripe.checkout.sessions.retrieve(session_id);
      console.log('  Stripe OK — payment_status:', session.payment_status, 'amount_total:', session.amount_total);
    } catch (stripeErr) {
    ...
    if (session.payment_status !== 'paid') {
      console.warn('  paiement non paid — statut:', session.payment_status);
      return new Response(JSON.stringify({ error: 'Paiement non confirmé : ' + session.payment_status }), { status: 402, headers: corsHeaders });
    }

    console.log('Etape 4: Connexion Supabase + recherche commande');
    ...
    if (findErr) {
      ...
      throw new Error('SELECT commandes : ' + findErr.message);
    }
    console.log('  commandes trouvées:', orders?.length ?? 0, orders?.[0] ? 'id=' + orders[0].id + ' statut=' + orders[0].statut : '');
    ...
    if (orders && orders.length > 0) {
      ...
      console.log('Etape 5: UPDATE pending_payment → en_attente id=', order.id);
      ...
      if (updateErr) {
        ...
        throw new Error('UPDATE commandes : ' + updateErr.message);
      }
      console.log('  UPDATE OK — result:', updated ? 'mis à jour' : 'race-condition (déjà activé)');
    ...
    // Fallback INSERT
    console.warn('Etape 5: FALLBACK INSERT — aucune commande pré-créée');
    ...
    if (insertErr) {
      ...
      throw new Error('INSERT commandes : ' + insertErr.message);
    }
